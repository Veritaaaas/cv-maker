...
import { faPlus, faXmark, faTrash, faBriefcase, faChevronDown, faChevronUp } from '@fortawesome/free-solid-svg-icons';
import './work.css';
...
function Work_experience({ works, setWorks }) {
  ...
  return (
    <div className="Experience">
      <div className='Experience-Header' onClick={() => setIsFormVisible(!isFormVisible)}>
        <h1><FontAwesomeIcon icon={faBriefcase} className='icon'/>Work Experience</h1>
        <FontAwesomeIcon icon={isFormVisible ? faChevronUp : faChevronDown}/>
      </div>
      {isFormVisible && (
        <>
          <div className="works-list">
            {!isAdding && works.map((work, index) => (
